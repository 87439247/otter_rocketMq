...
package com.alibaba.otter.node.etl.load.loader.rocketmq;

import com.alibaba.fastjson.JSONObject;
import com.alibaba.otter.node.common.config.ConfigClientService;
import com.alibaba.otter.node.etl.common.db.dialect.DbDialect;
import com.alibaba.otter.node.etl.common.rocketmq.RocketMQDialectFactory;
 ...
 * @version 4.0.0
 */
public class RocketMQLoadAction implements InitializingBean, DisposableBean {

    private static final Logger logger             = LoggerFactory.getLogger(RocketMQLoadAction.class);
    ...
    private RocketMQDialectFactory rocketMQDialectFactory;
...
//                    items = DbLoadMerger.merge(items);
    ...
    class DbLoadWorker implements Callable<Exception> {
        ...
        public DbLoadWorker(DbLoadContext context, List<EventData> datas, boolean canBatch){
            ...
            dbDialect = rocketMQDialectFactory.getDbDialect(context.getIdentity().getPipelineId(),
                ...
                while (true) {
                    try {
                        ...
                        for( EventData eventData : splitDatas ) {
                            logger.info("event data: {}", eventData.toString());
                            String mqTopic = context.getDataMediaSource().getName();
                            long executeTime = eventData.getExecuteTime();
                            long position = eventData.getPosition();
                            String schema = eventData.getSchemaName();
                            String table = eventData.getTableName();
                            String eventType = eventData.getEventType().getValue();
                            List<EventColumn> keyList = eventData.getKeys();
...
//                            Map<String, Object> json = new HashMap<String, Object>();
                            ...
                            json.put("B-SCHEMA",schema);
                            json.put("B-TABLE",table);
                            json.put("B-TIME",executeTime);
                            json.put("B-POS", position);
                            json.put("B-TYPE", eventType);

                            json.put("test", "又\u2028叫三秒补\uD83D\uDC4D水小绿瓶\u2028aaa");

                            for( EventColumn column : keyList ){
                                json.put(column.getColumnName(), column.getColumnValue());
                            }
                            for( EventColumn column : columnList ){
                                json.put(column.getColumnName(), column.getColumnValue());
                            }
//                            String body = objectMapper.writeValueAsString(json);
                            ...
                            logger.warn("send msg: {}, body: {}", message.toString(), body);
                            dbDialect.getRocketMQProducer().send(message);
                            // 更新统计信息
        ...
        private void processStat(EventData data, int affect, boolean batch) {
            if (batch && (affect < 1 && affect != Statement.SUCCESS_NO_INFO)) {
                ...
                RocketMQLoadAction.this.processStat(data, context);
            }
        }
    ...
    public void setRocketMQDialectFactory(RocketMQDialectFactory dbDialectFactory) {
        this.rocketMQDialectFactory = dbDialectFactory;
    }
